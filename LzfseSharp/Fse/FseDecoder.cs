...
internal static class FseDecoder
{
    ...
    public static int ValueDecode(ref ushort state, ReadOnlySpan<FseValueDecoderEntry> valueDecoderTable, ref FseInStream inStream)
    {
        ...
        return entry.ValueBase + (int)Core.BitOperations.ExtractBits(stateAndValueBits, 0, entry.ValueBits);
    }
    ...
    public static int InitDecoderTable(int stateCount, int symbolCount, ReadOnlySpan<ushort> freq, Span<int> table)
    {
        ...
        for (int i = 0; i < symbolCount; i++)
        {
            ...
            for (int j = 0; j < frequency; j++)
            {
                ...
                {
                    entryK = k;
                    entryDelta = ((frequency + j) << k) - stateCount;
                }
                else
                {
                    entryK = k - 1;
        ...
        for (int i = 0; i < symbolCount; i++)
        {
            ...
            for (int j = 0; j < frequency; j++)
            {
                ...
                if (j < j0)
                {
                    ...
                    entry.Delta = (short)(((frequency + j) << k) - stateCount);
                }
                else
                {
